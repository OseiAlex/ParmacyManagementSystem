  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("dispensary.sale.fetch"),
          data: {
            filter: {
    ...
    showFormModal(action, data) {
      ...
      if (this.action === "edit") {
        ...
        axios
          .post(route("dispensary.sale.edit"), { uuid: data })
          .then((response) => {
    ...
    submit() {
      if (!this.editContent) {
        this.form.post(route("dispensary.sale.store"), {
          onSuccess: () => {
            this.hideFormModal();
        ...
        this.form.put(route("dispensary.sale.update", { uuid: this.form.uuid }), {
          onSuccess: () => {
            this.hideFormModal();
    ...
    destroy() {
      axios
        .post(route("dispensary.sale.destroy"), {
