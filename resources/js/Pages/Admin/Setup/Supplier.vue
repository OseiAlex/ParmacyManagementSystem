  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.setup.supplier.fetch"),
        },
        columns: [
          { data: "company_name", title: "company name" },
          { data: "country", title: "country" },
          { data: "email", title: "company email" },
          { data: "phone", title: "company phone" },
          { data: "person_name", title: "person name" },
          { data: "person_email", title: "person email" },
          { data: "person_phone", title: "person phone" },
          {
            data: "action",
            name: "action",
            orderable: false,
    ...
    showFormModal(action, data) {
      ...
      if (this.action === "edit") {
        ...
        axios
          .post(route("admin.setup.supplier.edit"), {
            uuid: data,
          })
    ...
    submit() {
      if (!this.editContent) {
        this.form.post(route("admin.setup.supplier.store"), {
          onSuccess: () => {
            this.hideFormModal();
        ...
        this.form.put(route("admin.setup.supplier.update", { uuid: this.form.uuid }), {
          onSuccess: () => {
            this.hideFormModal();
    ...
    destroy() {
      axios
        .post(route("admin.setup.supplier.destroy"), {
          uuid: this.selectedRow,
        })
        .then((response) => [
          this.hideDestroyModal(),
          toastr.success("Supplier successfully removed"),
          this.fetch(),
        ])
...
<template>
  <Head title="Setup | Suppliers" />

  <AuthenticatedAdminLayout>
    <MenuDropdown ref="menuDropdown" />
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">Setup / Suppliers</h2>
    </template>
    ...
    <Modal
      :show="formModal"
      :closeable="true"
      :modalTitle="this.action + ' supplier'"
      @close="hideFormModal"
      :maxWidth="'md'"
    >
      <form @submit.prevent="submit">
        <div class="grid grid-cols-12 gap-5">
          <div class="col-span-full">
            <InputLabel for="name" value="company name" :required="true" />
            <TextInput
              id="name"
              type="text"
              class="w-full"
              ...
              autocomplete="name"
              :class="{ 'border-red-600': form.errors.company_name }"
            />
          ...
          <div class="col-span-full">
            ...
            <SelectInput
              ...
              class="w-full"
              :class="{ 'border-red-600': form.errors.country_id }"
            >
          ...
          <div class="col-span-full">
            <InputLabel for="tin" value="TIN" :required="false" />
            <TextInput
              id="tin"
              type="text"
              class="w-full"
              ...
              :placeholder="'TIN'"
              autocomplete="tin"
              :class="{ 'border-red-600': form.errors.tin }"
            />
            <InputError :message="form.errors.tin" />
          </div>

          <div class="col-span-full">
            <InputLabel for="person_name" value="Person name" :required="true" />
            <TextInput
              id="person_name"
              type="text"
              class="w-full"
              ...
              autocomplete="name"
              :class="{ 'border-red-600': form.errors.person_name }"
            />
            <InputError :message="form.errors.person_name" />
          </div>

          <div class="col-span-6">
            <InputLabel for="person_phone" value="person phone" :required="true" />
            <TextInput
              id="person_phone"
              type="text"
              class="w-full"
              ...
              autocomplete="phone"
              :class="{ 'border-red-600': form.errors.person_phone }"
            />
            <InputError :message="form.errors.person_phone" />
          </div>

          <div class="col-span-6">
            <InputLabel for="person_email" value="person email" :required="false" />
            <TextInput
              id="person_email"
              type="email"
              class="w-full"
              ...
              autocomplete="email"
              :class="{ 'border-red-600': form.errors.person_email }"
            />
            <InputError :message="form.errors.person_email" />
          </div>
