  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.setup.product.fetch"),
        },
        columns: [
    ...
    showFormModal(action, data) {
      ...
      if (this.action === "edit") {
        ...
        axios
          .post(route("admin.setup.product.edit"), {
            uuid: data,
          })
    ...
    submit() {
      if (!this.editContent) {
        this.form.post(route("admin.setup.product.store"), {
          onSuccess: () => {
            this.hideFormModal();
        ...
        this.form.put(route("admin.setup.product.update", { uuid: this.form.uuid }), {
          onSuccess: () => {
            this.hideFormModal();
    ...
    destroy() {
      axios
        .post(route("admin.setup.product.destroy"), {
    ...
    enlist() {
      axios
        .post(route("admin.setup.product.enlist"), {
          uuid: this.selectedRow,
        })
        .then((response) => [
...
<template>
  <Head title="Setup | Products" />

  <AuthenticatedAdminLayout>
    <MenuDropdown ref="menuDropdown" />
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">Setup / Products</h2>
    </template>
    ...
    >
      <form @submit.prevent="submit">
        <div class="grid grid-cols-12 gap-5">
          ...
          <div class="col-span-4">
            <InputLabel for="discount" value="discount (%)" :required="true" />
            <TextInput
              id="discount"
              type="text"
              class="w-full"
          ...
          <div class="col-span-4">
            <InputLabel for="markup_percentage" value="mark up (%)" :required="true" />
            <TextInput
