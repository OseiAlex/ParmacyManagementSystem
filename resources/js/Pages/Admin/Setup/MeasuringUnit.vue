  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.setup.measuring_unit.fetch"),
        },
        columns: [
    ...
    showFormModal(action, data) {
      ...
      if (this.action === "edit") {
        ...
        axios
          .post(route("admin.setup.measuring_unit.edit"), {
            uuid: data,
          })
    ...
    submit() {
      if (!this.editContent) {
        this.form.post(route("admin.setup.measuring_unit.store"), {
          onSuccess: () => {
            this.hideFormModal();
            toastr.success("Measuring Unit successfully saved");
            this.fetch();
          },
          onError: (errors) => {
        ...
        this.form.put(
          route("admin.setup.measuring_unit.update", { uuid: this.form.uuid }),
          {
            onSuccess: () => {
              this.hideFormModal();
              toastr.success("Measuring Unit successfully updated");
              this.fetch();
            },
            onError: (errors) => {
              toastr.error("Something went wrong");
    ...
    destroy() {
      axios
        .post(route("admin.setup.measuring_unit.destroy"), {
          uuid: this.selectedRow,
        })
        .then((response) => [
          this.hideDestroyModal(),
          toastr.success("Measuring Unit successfully removed"),
          this.fetch(),
        ])
...
<template>
  <Head title="Setup | Measuring Unit" />

  <AuthenticatedAdminLayout>
    <MenuDropdown ref="menuDropdown" />
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        Setup / Measuring Unit
      </h2>
    </template>
    ...
    <Modal
      :show="formModal"
      :closeable="true"
      :modalTitle="this.action + ' measuring unit'"
      @close="hideFormModal"
      :maxWidth="'md'"
    >
    ...
    <Modal
      :show="destroyModal"
      :closeable="true"
      :modalTitle="action + ' measuring unit'"
      @close="hideDestroyModal"
      :maxWidth="'md'"
    >
      <div class="flex justify-center mt-4">
        <p class="text-lg">Are you sure you want to delete this measuring unit?</p>
      </div>
